...
export default function Customers() {
  ...
  return (
    <ListViewLayout title="Customers" filterBar={filterBar} actions={actions}>
      {/* Your EXACT list block — unchanged */}
      <div className="panel mt-8" style={{ flex: 1, minHeight: 0 }}>
        <div className="panel-scroll">
          {/* Sticky header inside the scroller */}
          <div className="panel-header panel-header--frost hidden md:grid grid-cols-[1fr_240px_160px_1fr]">
          ...
          <div className="hidden md:grid text-sm grid-cols-[1fr_240px_160px_1fr]">
            {rows.map((r, i) => (
              <React.Fragment key={i}>
                ...
                <div className="px-4 py-3 text-white/80 border-b border-white/10 truncate">{r.email}</div>
                <div className="px-4 py-3 text-white/80 border-b border-white/10">{r.phone}</div>
                <div className="px-4 py-3 text-white/80 border-b border-white/10 truncate">{r.meta}</div>
              </React.Fragment>
            ))}
          </div>
        </div>
